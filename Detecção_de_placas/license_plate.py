...
def detection(image_1):
    ...
    plate_dimensions = (
    0.06 * label_image.shape[0], 0.13 * label_image.shape[0], 0.085 * label_image.shape[1], 0.2 * label_image.shape[1])
    plate_dimensions2 = (
    0.06 * label_image.shape[0], 0.18 * label_image.shape[0], 0.15 * label_image.shape[1], 0.25 * label_image.shape[1])
    ...
    for region in regionprops(label_image):
        ...
        if (
                region_height >= min_height and region_height <= max_height and region_width >= min_width and region_width <= max_width and region_width > region_height):
            flag = 1
            go = go + 1
            print(go)
    ...
    if (len(plate_like_objects) != 0):
        ...
        th_2 = cv2.adaptiveThreshold(img_3, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11,
                                     2)  # this one is better after all the thresholdings
        ...
        0.1875 * license_plate.shape[0], 0.480 * license_plate.shape[0], 0.05 * license_plate.shape[1],
        0.40 * license_plate.shape[1])
        ...
        for regions in regionprops(labelled_plate):
            ...
            if (
                    region_height > min_height and region_height < max_height and region_width > min_width and region_width < max_width):
...
def trackMultipleObjects():
    ...
    out = cv2.VideoWriter('C:\\Users\\SHREYA\\Desktop\\outpy.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 10,
                          (WIDTH, HEIGHT))
    ...
    while True:
        ...
        if not (frameCounter % 10):  # it goes inside only when frameCounter is a multiple of 10
            ...
            for i in range(len(cars)):
                ...
                if (cars[i][0] < 195) or (cars[i][2] < 58) or (cars[i][3] < 58) or (
                        (cars[i][1] > 40) and (cars[i][2] < 68)) or ((cars[i][1] > 268) and (cars[i][2] < 250)) or (
                        (cars[i][1] > 268) and (cars[i][3] < 250)):
                    myuse.append(i)
        ...
        for carID in carTracker.keys():
            ...
            for carID_2 in carTracker.keys():
                ...
                if (carID != carID_2):
                    ...
                    if (x_2 < (x_1 + 10)) and ((x_1 + 10) < (x_2 + w_2)) and ((x_1 + w_1 - 10) > x_2) and (
                            (x_1 + w_1 - 10) < (x_2 + w_2)) and ((y_1 + h_1 + 10) > y_2) and (
                            (y_1 + h_1 + 10) < (y_2 + h_2)):
                        myuse.append(carID)
                        # print('HAKUNAAAAA')
        ...
        for carID in carTracker.keys():
            ...
            for carID_2 in carTracker.keys():
                ...
                if (carID != carID_2):
                    ...
                    if ((x_1 + w_1 + 10) > x_2) and ((x_1 + w_1 + 10) < (x_2 + w_2)) and ((y_1 + 10) > y_2) and (
                            (y_1 + 10) < (y_2 + h_2)) and ((y_1 + h_1 - 10) > y_2) and ((y_1 + h_1 - 10) < (y_2 + h_2)):
                        myuse.append(carID)
        ...
        for carID in carTracker.keys():
            ...
            for carID_2 in carTracker.keys():
                ...
                if (carID != carID_2):
                    ...
                    if ((x_1 + 10) > x_2) and ((x_1 + 10) < (x_2 + w_2)) and ((x_1 + w_1 - 10) > x_2) and (
                            (x_1 + w_1 - 10) < (x_2 + w_2)) and ((y_1 - 10) > y_2) and ((y_1 - 10) < (y_2 + h_2)):
                        myuse.append(carID)
        ...
        for carID in carTracker.keys():
            ...
            for carID_2 in carTracker.keys():
                ...
                if (carID != carID_2):
                    ...
                    if ((x_1 - 10) > x_2) and ((x_1 - 10) < (x_2 + w_2)) and ((y_1 + 10) > y_2) and (
                            (y_1 + 10) < (y_2 + h_2)) and ((y_1 + h_1 - 10) > y_2) and ((y_1 + h_1 - 10) < (y_2 + h_2)):
                        myuse.append(carID)
        ...
        for i in carLocation1.keys():
            if frameCounter % 1 == 0:  # it will always work
                ...
                if [x1, y1, w1, h1] != [x2, y2, w2, h2]:
                    if (speed[i] == None or speed[
                        i] == 0):  # 275.. 285  and y1 >= 250 and y1 <= 260.. this is to give command to the code to only estimate speed when the vehicle is between 275 and 285!
                        speed[i] = estimateSpeed([x1, y1, w1, h1], [x2, y2, w2, h2], fps)
                        print('1st', x1, y1, w1, h1, ' ', '2nd', x2, y2, w2, h2, ' ', speed[i], ' ', 'fps:', fps)
                        # print('')

                    # if y1 > 275 and y1 < 285:
                    if (speed[i] != None and (x2 > x1 + 5 or x2 < x1 - 5) and (
                            y2 > y1 + 5 or y2 < y1 - 5)):  # so that even if the driver opens his seat, the code doesn't detect speed in that
